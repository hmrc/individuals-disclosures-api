...
class GivenNameValidationSpec extends UnitSpec with Inspectors {
  val validator: GivenNameValidation.type = GivenNameValidation

  "GivenNameValidation" must {
    "allow alphanumeric values with arbitrary capitalization" in {
      forAll(Seq("alice", "Bob")) { name =>
        validator.isValid(name) shouldBe true
      }
    }

    "allow hyphenated and spaces and dots" in {
      forAll(Seq("Tommy-Lee", "Mary Jane", "will.i")) { name =>
        validator.isValid(name) shouldBe true
      }
    }
    ...
    //    "allow diacritics" in {
    //      forAll(Seq("Arsène", "José", "Jokūbas", "Jørgen")) { name =>
    //        validator.isValid(name) shouldBe true
    //      }
    //    }

    "allow ordinals" in {
      forAll(Seq("Henry 8th")) { name =>
        validator.isValid(name) shouldBe true
      }
    }
