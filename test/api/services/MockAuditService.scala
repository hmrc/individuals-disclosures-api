...
trait MockAuditService extends TestSuite with MockFactory {
  ...
  object MockedAuditService {
    ...
    def verifyAuditEvent[T](event: AuditEvent[T]): CallHandler[Future[AuditResult]] =
      verifyAuditEvent(event, Future.successful(AuditResult.Success))

    def verifyAuditEvent[T](event: AuditEvent[T], result: Future[AuditResult]): CallHandler[Future[AuditResult]] = {
      (mockAuditService
